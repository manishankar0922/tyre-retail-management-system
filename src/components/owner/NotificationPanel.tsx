...
import { AlertTriangle, Receipt, RefreshCw, ShoppingBag } from "lucide-react";
import { formatIST } from "@/utils/date";

export interface NotificationEvent {
  id: string;
  type: "low_stock" | "stock_update" | "invoice_generated" | "purchase_added";
  message: string;
  created_at: string;
}
      ...
      <div className="overflow-y-auto custom-scrollbar p-3 space-y-2 flex-1 min-h-0">
        {isLoading ? (
          ...
          notifications.map((item) => (
            <div
              key={item.id}
              className="p-2.5 rounded-xl bg-zinc-50/50 dark:bg-zinc-900/30 border border-zinc-200/40 dark:border-zinc-800/40 hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors flex gap-2.5 text-[11px] font-semibold text-zinc-700 dark:text-zinc-300"
            >
